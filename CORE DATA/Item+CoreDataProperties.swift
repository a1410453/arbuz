...
extension Item : Identifiable {
    ...
    static func with(name: String,
                     _ context:NSManagedObjectContext) -> Item? {
        ...
        do {
            guard let results = try context.fetch(request) as? [Item],
                  results.count == 1,
                  let item = results.first
            else { return Item(context: context) }
            return item
        } catch (let error){
    ...
    class func readAll(_ context:NSManagedObjectContext) -> [Item]? {
        let request = Item.request()
        do {
            guard let results = try context.fetch(request) as? [Item],
